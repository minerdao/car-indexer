...
export async function extractCarFilesAndOutput(carFiles: string[], outputPath: string, chunkSize = 10): Promise<void> {
  ...
  for (let i = 0; i < carFiles.length; i += chunkSize) {
    ...
    for (const item of chunkResults) {
      writeStream.write(`${item.pieceCID},${item.fileSize},${item.pieceSize},${item.dataCID}\n`)
    }
  }
...
console.log(`${carFiles.length} car files in total`)

extractCarFilesAndOutput(carFiles, outputFile)
.then(() => {
